...
if menu == "DASHBOARD":
    ...
    with conectar_db() as conn:
        ...
        if ubis_db:
            ...
            for u in ubis_db[inicio_u : inicio_u + ITEMS_POR_PAGINA]:
                col_i, col_e, col_d = st.columns([4, 0.5, 0.5])
                col_i.write(f"🚩 **{u[1]}** ➔ {u[0]}")
                if col_e.button("✏️", key=f"ed_u_{u[0]}_{u[1]}"): editar_ubicacion_dialog(u[0], u[1])
                if col_d.button("🗑️", key=f"de_u_{u[0]}_{u[1]}"): confirmar_eliminacion_ubi(u[0], u[1])
