...
import pandas as pd
import base64
import os  # <--- AGREGADO: Necesario para Railway
...
# Cargamos la imagen
# NOTA IMPORTANTE: Asegúrate de que 'logo1.png' y 'logo.png' estén subidos en tu GitHub.
try:
    favicon = Image.open("logo1.png")
except FileNotFoundError:
    favicon = None # Evita que la app se rompa si falta la imagen
...
def get_engine():
    try:
        ...
        # 1. Intentamos leer la variable de entorno de Railway llamada 'DATABASE_URL'
        db_url = os.environ.get("DATABASE_URL")
        
        # 2. Si no existe (estamos en local), intentamos leer secrets.toml
        if not db_url:
            if "connections" in st.secrets and "postgresql" in st.secrets["connections"]:
                ...
                # Si falla todo, retornamos None
                return None
        
        # Corrección para Supabase: SQLAlchemy necesita que empiece por postgresql://
...
if menu == "DASHBOARD":
    ...
    def cargar_datos_activos():
        return read_query("SELECT * FROM activos")

    df = cargar_datos_activos()
    
    
    # Cargamos TODO de una vez para filtrar en memoria (más rápido y seguro)
    ...
    # 2. SELECTOR DE CATEGORÍA (NIVEL SUPERIOR)
    f_cat = st.selectbox("**SELECCIONAR CATEGORÍA**", ["SELECCIONAR"] + CATEGORIAS_LISTA)

    if f_cat != "SELECCIONAR":
        # Filtramos el DataFrame principal por categoría
        df_cat = df[df['categoria'] == f_cat]
        
        st.markdown("---")
        st.markdown("### 🔍 Buscar por:")
        
        # 3. FILTROS GLOBALES (Fuera de las pestañas)
        # Esto asegura que los filtros apliquen a TODO antes de calcular nada
        c_filt1, c_filt2, c_filt3 = st.columns(3)
        
        # A. Filtro de Estado
        val_estado = c_filt1.selectbox("**ESTADO**", ["TODOS", "OPERATIVO", "DAÑADO", "REPARACION"])
        
        # B. Filtro de Ubicación (Unificado)
        # Creamos una lista inteligente: "NOMBRE UBICACION (PAIS)" para distinguir
        opciones_ubis = ["TODAS"]
        if not df_todas_ubis.empty:
            # Creamos formato "ALMACEN (VENEZUELA)"
            opciones_ubis += [f"{row['nombre']} ({row['pais']})" for _, row in df_todas_ubis.iterrows()]
        
        val_ubicacion_raw = c_filt2.selectbox("**UBICACIÓN**", opciones_ubis)
        
        # C. Filtro de Búsqueda
        val_busqueda = c_filt3.text_input("**CÓDIGO, MARCA O PLACA**").upper()

        # 4. APLICACIÓN DE FILTROS (MOTOR ÚNICO)
        # Creamos 'df_filtrado' que será la ÚNICA fuente de verdad para contadores y tablas
        df_filtrado = df_cat.copy()

        # -> Aplicar Estado
        if val_estado != "TODOS":
            df_filtrado = df_filtrado[df_filtrado['estado'] == val_estado]

        # -> Aplicar Ubicación
        if val_ubicacion_raw != "TODAS":
            # Extraemos solo el nombre de la ubicación, quitando el " (PAIS)" del final
            # Ejemplo: "ALMACEN (VENEZUELA)" -> "ALMACEN"
            nombre_ubi_real = val_ubicacion_raw.split(" (")[0]
            pais_ubi_real = val_ubicacion_raw.split(" (")[1].replace(")", "")
            
            # Filtramos por nombre Y país para ser exactos
            df_filtrado = df_filtrado[
                (df_filtrado['ubicacion'] == nombre_ubi_real) & 
                (df_filtrado['pais'] == pais_ubi_real)
            ]

        # -> Aplicar Búsqueda
        if val_busqueda:
            df_filtrado = df_filtrado[
                df_filtrado['id'].astype(str).str.contains(val_busqueda, na=False) | 
                df_filtrado['marca'].str.contains(val_busqueda, na=False) |
                df_filtrado['placa'].astype(str).str.contains(val_busqueda, na=False)
            ]

        # 5. CÁLCULO DE MÉTRICAS (Basado en df_filtrado)
        # Al usar df_filtrado, los números son consecuencia directa de los filtros de arriba
        total_activos = len(df_filtrado)
        count_vzla = len(df_filtrado[df_filtrado['pais'] == "VENEZUELA"])
        count_col = len(df_filtrado[df_filtrado['pais'] == "COLOMBIA"])
        count_usa = len(df_filtrado[df_filtrado['pais'] == "ESTADOS UNIDOS"])

        st.markdown("---")
        
        # Visualización de Contadores
        cm1, cm2, cm3, cm4 = st.columns(4)
        cm4.metric("TOTAL", total_activos)
        cm1.metric("🇻🇪 VENEZUELA", count_vzla)
        cm2.metric("🇨🇴 COLOMBIA", count_col)
        cm3.metric("🇺🇸 EE.UU.", count_usa)

        st.markdown("---")

        # 6. VISUALIZACIÓN DE PESTAÑAS
        # Aquí simplemente mostramos lo que ya filtramos
        tabs_paises = st.tabs(PAISES_LISTA)

        for i, pais_nombre in enumerate(PAISES_LISTA):
            with tabs_paises[i]:
                # Sub-filtramos el dataframe maestro por el país de la pestaña
                df_tab = df_filtrado[df_filtrado['pais'] == pais_nombre]
                
                if df_tab.empty:
                    st.info(f"No hay activos en **{pais_nombre}** que coincidan con los filtros de arriba.")
                else:
                    # --- LÓGICA DE PAGINACIÓN ---
                    items_por_pag = ITEMS_POR_PAGINA
                    pag_key = f"pag_dash_new_{pais_nombre}_{f_cat}" # Nueva key para evitar conflictos
                    
                    if pag_key not in st.session_state: st.session_state[pag_key] = 0
                    
                    total_rows = len(df_tab)
                    total_pags = (total_rows - 1) // items_por_pag + 1
                    
                    if st.session_state[pag_key] >= total_pags: st.session_state[pag_key] = 0
                        
                    inicio = st.session_state[pag_key] * items_por_pag
                    fin = inicio + items_por_pag
                    
                    df_pagina = df_tab.iloc[inicio:fin]
                    
                    st.caption(f"Mostrando {len(df_pagina)} de {total_rows} activos encontrados.")

                    # --- RENDERIZADO DE CARTAS (Mismo código de siempre) ---
                    for _, row in df_pagina.iterrows():
                        color = "🟢" if row['estado'] == "OPERATIVO" else "🔴" if row['estado'] == "DAÑADO" else "🟡"
                        titulo_expander = f"{color} **ID: {row['id']}**"
                        if row['placa']: titulo_expander += f"| **PLACA: {row['placa']}**"
                        
                        with st.expander(titulo_expander):
                            
                            # (AQUI PEGA TU BLOQUE DE EDICION / VISUALIZACION INTERNA)
                            # Para mantener la respuesta limpia, uso la misma lógica exacta que tenías
                            # -----------------------------------------------------------------------
                            if f"edit_{row['id']}" in st.session_state:
                                with st.form(f"form_edit_{row['id']}"):
                                    ...
                                    # Archivos
                                    st.write("---")
                                    st.write("🗑️ **ELIMINAR ARCHIVOS EXISTENTES** (Solo marcar los que desea borrar)")
                                    ...
                                    for idx, f_row in fotos_actuales.iterrows():
                                        if st.checkbox(f"**BORRAR FOTO:** {f_row['nombre_archivo']}", key=f"d_f_{f_row['id']}"): eliminar_fotos_ids.append(f_row['id'])
                                    eliminar_docs_ids = []
                                    for idx, d_row in docs_actuales.iterrows():
                                        if st.checkbox(f"**BORRAR DOCUMENTO:** {d_row['nombre_archivo']}", key=f"d_d_{d_row['id']}"): eliminar_docs_ids.append(d_row['id'])
                                    ...
                                    if st.form_submit_button("💾 GUARDAR CAMBIOS"):
                                        ...
                                        run_query("""UPDATE activos SET marca=:ma, modelo=:mo, estado=:st, motivo_estado=:mst, 
                                                 ...
                                                 {"ma": emarc, "mo": emod, "st": eest, "mst": emot, "ub": eubi, "de": edesc, "ca": ecat, "ur": erev, "pa": epais, "id": row['id']})
                                ...
                                if st.button("CANCELAR", key=f"can_{row['id']}"): del st.session_state[f"edit_{row['id']}"]; st.rerun()
                                ...
                                with col_img:
                                    ...
                                    if not fotos_df.empty:
                                        ...
                                        st.image(bytes(fotos_df.iloc[idx % len(fotos_df)]['datos_binarios']), use_container_width=True)
                                        ca, cb = st.columns(2)
                                    ...
                                    else: st.info("**Sin foto registrada.**")
                                ...
                                with col_info:
                                    ...
                                    if not docs_df.empty:
                                        for _, d_row in docs_df.iterrows():
                                            if st.button(f"📄 {d_row['nombre_archivo']}", key=f"v_{d_row['id']}"): visor_documento(d_row['nombre_archivo'], d_row['datos_binarios'])
                                    st.divider()

                                    c_b1, c_b2 = st.columns(2)
                                    if c_b1.button("✏️ EDITAR ACTIVO", key=f"be_{row['id']}", use_container_width=True): st.session_state[f"edit_{row['id']}"] = True; st.rerun()
                                    if c_b2.button("🗑️ ELIMINAR ACTIVO", key=f"bd_{row['id']}", use_container_width=True): confirmar_eliminar_activo(row['id'])
                    ...
                    if total_pags > 1:
                        ...
                        if st.session_state[pag_key] > 0 and c_nav1.button("⬅️ Ant.", key=f"bp_{pais_nombre}"):
                            st.session_state[pag_key] -= 1; st.rerun()
                        if st.session_state[pag_key] < total_pags - 1 and c_nav3.button("Sig. ➡️", key=f"bn_{pais_nombre}"):
                            st.session_state[pag_key] += 1; st.rerun()
    else:
