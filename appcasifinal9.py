...
# Se asume que tienes configurado .streamlit/secrets.toml
def get_engine():
    try:
...
def inicializar_db():
    # PostgreSQL usa BYTEA para archivos binarios
    queries = [
        ...
        # Modificado: Se guardan los datos binarios, no el path
    ...
    with engine.connect() as conn:
        ...
        # Actualización de columnas existentes (Postgres way)
...
# --- FUNCIONES DE APOYO (MODIFICADAS PARA NUBE) ---
...
@st.dialog("VISOR")
def visor_documento(nombre, datos_bytes):
    st.write(f"### {nombre}")
                        ...
                        with st.expander(f"{color} ID: {row['id']} | {row['categoria']} | {row['marca']}"):
                            ...
                            if f"edit_{row['id']}" in st.session_state:
                                with st.form(f"form_edit_{row['id']}"):
                                    ...
                                    # Recargar ubicaciones basadas en el pais seleccionado para editar
                                    ...
                                    # Cargar fotos/docs desde BD para listar y eliminar
                                ...
                                with col_img:
                                    ...
                                    if not fotos_df.empty:
                                        ...
                                        # Convertir bytes a imagen para st.image (Streamlit lee bytes directamente)
                                        st.image(fotos_df.iloc[idx % len(fotos_df)]['datos_binarios'], use_container_width=True)
...
#--- REGISTRAR ACTIVO ---
    ...
    st.title("📝 REGISTRAR NUEVO ACTIVO")
        ...
        if st.button("💾 GUARDAR", use_container_width=True):
            if rid and ubis_filtradas and rubi != "SIN UBICACIÓN" and rcat and (rest == "OPERATIVO" or rmot):
                try:
                    ...
                    st.error(f"❌ Error al guardar (¿ID Duplicado?): {e}")
            else:
