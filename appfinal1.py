                ...
                if df_display.empty:
                    ...
                    for _, row in df_pagina.iterrows():
                        ...
                        with st.expander(f"{color} ID: {row['id']} | {row['categoria']} | {row['marca']}"):
                            ...
                            with col_img:
                                # ... (tu código de imágenes anterior se mantiene igual) ...
                            ...
                            with col_info:
                                ...
                                # Verifica si existe 'motivo_estado' y si no está vacío
...
#--- REGISTRAR ACTIVO ---
    ...
    def resetear_formulario():
        # 1. Para textos y selectores: Forzamos el valor a vacío/None
        st.session_state["reg_id"] = ""
        st.session_state["reg_placa"] = ""
        st.session_state["reg_marc"] = ""
        st.session_state["reg_mod"] = ""
        st.session_state["reg_mot"] = ""
        st.session_state["reg_desc"] = ""
        
        st.session_state["reg_cat"] = None
        st.session_state["reg_pais"] = None
        st.session_state["reg_ubi"] = None
        st.session_state["reg_est"] = "OPERATIVO" # Lo devolvemos al valor por defecto
        
        # 2. Para archivos: Es necesario eliminar la clave para que el widget se resetee
        if "reg_fotos" in st.session_state: del st.session_state["reg_fotos"]
        if "reg_docs" in st.session_state: del st.session_state["reg_docs"]
            ...
            if not (rid and ubis_filtradas and rubi and rubi != "SIN UBICACIÓN" and rcat and (rest == "OPERATIVO" or rmot)):
                ...
                if problemas:
                    ...
                    try:
                        ...
                        # LLAMADA A LA FUNCIÓN DE LIMPIEZA ANTES DEL RERUN
                        resetear_formulario()
